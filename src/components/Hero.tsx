      ...
      <motion.div initial={reduce ? false : { opacity: 0, y: 26 }} animate={start ? { opacity: 1, y: 0 } : { opacity: 0, y: 26 }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="relative z-10 mx-auto mt-8 flex w-full min-w-0 max-w-6xl flex-col items-center text-center">
        ...
        <div className="mt-12 h-20 w-full max-w-3xl opacity-75">
          <LogoLoop
            ...
            scaleOnHover
            fadeOut
            fadeOutColor="var(--bg-hero)"
            ariaLabel="Technology stack"
          />
        </div>
      </motion.div>
      <motion.a href="#expertise" aria-label="Scroll to expertise" initial={reduce ? false : { opacity: 0, y: 12 }} animate={start ? { opacity: 1, y: 0 } : { opacity: 0, y: 12 }} transition={{ delay: start ? 1 : 0 }} className="absolute bottom-8 left-1/2 grid h-16 w-9 -translate-x-1/2 place-items-center rounded-full border-2 border-white/75 text-accent">
...
function TypewriterSubtitle({ text, reduce, start }: { text: string; reduce: boolean; start: boolean }) {
  ...
  }, [reduce, start, text, keypress]);

  const isTyping = typed.length < text.length;

  return (
    <p aria-label={text} className="pixel-copy mx-auto mt-8 grid max-w-2xl text-[12px] font-bold uppercase tracking-[0.18em] text-accent">
      <span className="invisible col-start-1 row-start-1" aria-hidden="true">{text}</span>
      <span className="col-start-1 row-start-1" aria-hidden="true">
        {typed}
        {isTyping ? <span className="ml-1 inline-block h-[1em] w-[0.08em] translate-y-[0.12em] bg-accent" /> : null}
      </span>
    </p>
  );
}
