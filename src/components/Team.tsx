...
function TeamPhoto({ name, src }: { name: string; src?: string }) {
  ...
  return (
    <div className="grid h-full w-full place-items-center bg-[var(--bg-elevated)] text-2xl font-black tracking-[-0.08em] text-white">
      {initials(name)}
    </div>
  );
}
...
export function Team() {
  ...
  return (
    <section id="team" className="binary-surface relative overflow-hidden px-4 py-24 sm:py-32">
      <BinaryBackground section="team" />
      <div className="studio-panel relative mx-auto max-w-6xl overflow-hidden px-5 py-10 sm:px-10 sm:py-14 lg:px-14">
          ...
          <div className="lg:sticky lg:top-28">
            ...
            >
              {titleWords.map((word, index) => (
                <motion.span
                  key={word}
                   className={`block text-6xl font-black leading-[0.9] tracking-[-0.075em] sm:text-8xl ${index === 2 ? 'text-accent' : 'text-white'}`}
                ...
                return (
                  ...
                  >
                    <div className="relative flex shrink-0 flex-col items-center self-stretch">
                      <motion.div
                        className={`${isLead ? 'h-24 w-24 md:h-28 md:w-28' : 'h-20 w-20 md:h-24 md:w-24'} relative z-10 shrink-0 overflow-hidden rounded-full border-2 border-[var(--border-default)] bg-[var(--bg-main)]`}
                    ...
                    <div className={`studio-card min-w-0 flex-1 p-5 ${isLead ? 'md:p-6' : ''}`}>
                      ...
                      <p className="mt-1 text-sm font-bold text-zinc-300">{member.role}</p>
